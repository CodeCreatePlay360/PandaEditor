...
def GetGeomTriangle(v1, v2, v3):
    tri = p3d_core.GeomTriangles(p3d_core.Geom.UHDynamic)
    tri.addVertex(v1)
    tri.addVertex(v2)
    tri.addVertex(v3)
    tri.closePrimitive()
...
class Arc(p3d_core.NodePath):
    """NodePath class representing a wire arc."""

    def __init__(self, radius=1.0, numSegs=16, degrees=360, axis=p3d_core.Vec3(1, 0, 0),
                 thickness=1.0, origin=p3d_core.Point3(0, 0, 0)):
        # Create line segments
        self.ls = p3d_core.LineSegs()
        ...
        for p in GetPointsForArc(degrees, numSegs):
            ...
            p = p3d_core.Point3(p[0], p[1], 0) - origin
            p = RotatePoint3(p, p3d_core.Vec3(0, 0, 1), p3d_core.Vec3(axis))
            self.ls.drawTo(p * radius)

        # Init the node path, wrapping the lines
        node = self.ls.create()
        p3d_core.NodePath.__init__(self, node)


def Circle(radius=1.0, numSegs=16, axis=p3d_core.Vec3(1, 0, 0),
           thickness=1.0, origin=p3d_core.Point3(0, 0, 0)):
    # Create line segments
    ls = p3d_core.LineSegs()
...
def Square(width=1, height=1, axis=p3d_core.Vec3(1, 0, 0), thickness=1.0, origin=p3d_core.Point3(0, 0, 0)):
    ...
    for p in points:
        # Draw the point rotated around the desired axis
        p = p3d_core.Point3(p[0], p[1], 0) - origin
        p = RotatePoint3(p, p3d_core.Vec3(0, 0, 1), axis)
        ls.drawTo(p)
...
def Cone(radius=1.0, height=2.0, numSegs=16, degrees=360,
         axis=p3d_core.Vec3(0, 0, 1), origin=p3d_core.Point3(0, 0, 0)):
    """Return a geom node representing a cone."""
    # Create vetex data format
    gvf = p3d_core.GeomVertexFormat.getV3n3()
    gvd = p3d_core.GeomVertexData('vertexData', gvf, p3d_core.Geom.UHStatic)

    # Create vetex writers for each type of data we are going to store
    gvwV = p3d_core.GeomVertexWriter(gvd, 'vertex')
    gvwN = p3d_core.GeomVertexWriter(gvd, 'normal')

    # Get the points for an arc
    axis2 = p3d_core.Vec3(axis)
    axis2.normalize()
    ...
    for i in range(len(points) - 1):
        ...
        p1 = p3d_core.Point3(points[i][0], points[i][1], 0) * radius
        p1 = RotatePoint3(p1, p3d_core.Vec3(0, 0, 1), axis) - origin
        p2 = p3d_core.Point3(points[i + 1][0], points[i + 1][1], 0) * radius
        p2 = RotatePoint3(p2, p3d_core.Vec3(0, 0, 1), axis) - origin
    ...
    geomNode = p3d_core.GeomNode('cone')
...
def Cylinder(radius=1.0, height=2.0, numSegs=16, degrees=360,
    ...
    geomNode = p3d_core.GeomNode('cylinder')
    geomNode.addGeom(geom)
    return geomNode
...
def Sphere(radius=1.0, numSegs=16, degrees=360,
    ...
    for z in range(1, len(zPoints) - 2):
        ...
        for i in range(len(points) - 1):
            ...
            # Get points
            p1 = p3d_core.Point3(points[i][0], points[i][1], 0) * rad1
            p2 = p3d_core.Point3(points[i + 1][0], points[i + 1][1], 0) * rad1
            p3 = p3d_core.Point3(points[i + 1][0], points[i + 1][1], 0) * rad2
            p4 = p3d_core.Point3(points[i][0], points[i][1], 0) * rad2

            # Rotate the points around the desired axis
            p1, p2, p3, p4 = [
                RotatePoint3(p, p3d_core.Vec3(0, 0, 1), axis)
...
def Box(width=1, depth=1, height=1, origin=p3d_core.Point3(0, 0, 0)):
    ...
    for p in GetPointsForBox(width, depth, height):
        gvwV.addData3f(p3d_core.Point3(p) - origin)
    ...
    geomNode = p3d_core.GeomNode('box')
    geomNode.addGeom(geom)
    return geomNode


class Polygon(p3d_core.NodePath):

    def __init__(self, points, normals=None, texcoords=None, origin=p3d_core.Point3(0, 0, 0)):

        # Create vetex data format
        gvf = p3d_core.GeomVertexFormat.getV3n3t2()
        gvd = p3d_core.GeomVertexData('vertexData', gvf, p3d_core.Geom.UHStatic)

        # Create vetex writers for each type of data we are going to store
        gvwV = p3d_core.GeomVertexWriter(gvd, 'vertex')
        gvwN = p3d_core.GeomVertexWriter(gvd, 'normal')
        gvwT = p3d_core.GeomVertexWriter(gvd, 'texcoord')

        # Write out all points and normals
        for i, point in enumerate(points):
            p = p3d_core.Point3(point) - origin
            gvwV.addData3f(p)
        ...
        geomNode = p3d_core.GeomNode('poly')
        geomNode.addGeom(geom)
        p3d_core.NodePath.__init__(self, geomNode)
...
def Line(start, end, thickness=1.0):
    ...
    ls = p3d_core.LineSegs()
    ls.setThickness(thickness)
    ls.drawTo(p3d_core.Point3(start))
    ls.drawTo(p3d_core.Point3(end))
...
def QuadWireframe(egg):
    def RecursePoly(node, geo):
        ...
        if isinstance(node, p3d_core.EggPolygon):

            # Get each vert position
            poss = []
            for vert in node.getVertices():
                pos3 = vert.getPos3()
                pos = p3d_core.Point3(pos3.getX(), pos3.getY(), pos3.getZ())
                poss.append(pos)
    ...
    return RecursePoly(egg, p3d_core.GeomNode('quadWireframe'))
