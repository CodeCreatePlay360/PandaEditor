...
from game.resources import RuntimeModule, EditorPlugin, Component
...
Model_icon = str(pathlib.Path(constants.ICONS_PATH + "/resource tiles panel/3D-objects-icon.png"))
Texture_icon = str(pathlib.Path(constants.ICONS_PATH + "/resource tiles panel/images.png"))
Sound_icon = str(pathlib.Path(constants.ICONS_PATH + "/resource tiles panel/music.png"))
Script_icon = str(pathlib.Path(constants.ICONS_PATH + "/resource tiles panel/script_code.png"))
Video_icon = str(pathlib.Path(constants.ICONS_PATH + "/resource tiles panel/video.png"))
...
class ImageTile(wx.Panel):
    ...
    def load_model(self):
        pass
        # editor.command_mgr.do(commands.LoadModel(path=""))
...
class ImageTilesPanel(ScrolledPanel):
    ...
    def __init__(self, parent):
        ...
        self.__gridSizer = wx.GridSizer(1, 1, 1, 0)
    ...
    def set_from_selections(self, selections, organize=True):
        self.Freeze()
        ...
        for dir_name, path in selections:
            ...
            for item in dir_items:
                ...
                if extension in self.supported_extensions_to_icons_map:
                    image = self.get_image(path, extension)
                    if organize:
                        selections_organized[extension].append((image, file_name, extension, file_path))
                    else:
                        self.create_tile(image, file_name, extension, file_path)
                else:
    ...
    def update_tiles(self):
        ...
        self.__gridSizer.SetCols(tiles_per_row)
        self.__gridSizer.SetRows(num_rows)
    ...
    def get_image(self, path, extension):
        if editor.level_editor.is_module(path):
            module = editor.level_editor.get_module(path)
