...
class BaseNodePath:
    ...
    def create_properties(self):
        ...
        label = EditorProperty.Label(name="Transform", is_bold=True)

        pos = EditorProperty.FuncProperty(name="Position",
                                          initial_value=self.np.getPos(),
                                          setter=self.np.setPos,
                                          getter=self.np.getPos,
                                          length=36)

        rot = EditorProperty.FuncProperty(name="Rotation",
                                          initial_value=self.np.getHpr(),
                                          setter=self.np.setHpr,
                                          getter=self.np.getHpr,
                                          length=36)

        scale = EditorProperty.FuncProperty(name="Scale",
                                            initial_value=self.np.getScale(),
        ...
        label = EditorProperty.Label(name="Node Properties", is_bold=True)
        color = EditorProperty.FuncProperty(name="Color",
                                            initial_value=self.np.get_color(),
